...
async def test_e2e_proactive_maintenance_flow(monkeypatch):
    ...
    if os.path.exists(test_sqlite): os.remove(test_sqlite)
    if os.path.exists(test_duck): os.remove(test_duck)
    ...
    heartbeat = HeartbeatManager(agent, interval_seconds=0.1, im_provider=im_provider)
    ...
    with open("maintenance_fixed.txt", "r") as f:
