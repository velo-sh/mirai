...
from mirai.db.session import init_db, async_session
...
async def test_memory_isolation_between_collaborators():
    ...
    async with async_session() as session:
        ...
        if not await manager.get_collaborator(collab_a_id):
            await manager.create_collaborator(CollaboratorCreate(
                id=collab_a_id, name="Alice", role="QA", system_prompt="Alice prompt"
            ))
        if not await manager.get_collaborator(collab_b_id):
            await manager.create_collaborator(CollaboratorCreate(
                id=collab_b_id, name="Bob", role="Dev", system_prompt="Bob prompt"
            ))
    ...
    alice_agent = await AgentLoop.create(
        provider=provider, 
        tools=[MemorizeTool(collaborator_id=collab_a_id)], 
    ...
    bob_agent = await AgentLoop.create(
        provider=provider, 
        tools=[MemorizeTool(collaborator_id=collab_b_id)], 
    ...
    bob_memories = await bob_agent.l2_storage.search(
        vector=query_vector, 
        limit=5, 
        filter=f"collaborator_id = '{collab_b_id}'"
    )
    ...
    alice_memories = await alice_agent.l2_storage.search(
        vector=query_vector_alice, 
        limit=5, 
        filter=f"collaborator_id = '{collab_a_id}'"
    )
