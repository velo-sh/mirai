...
async def test_dreaming_flow():
    ...
    # Clean start
    if os.path.exists("mirai_hdd.duckdb"): os.remove("mirai_hdd.duckdb")
    if os.path.exists("mirai_vectors"): shutil.rmtree("mirai_vectors")
    await init_db()
    ...
    has_secret = any("secret password" in json.loads(r["metadata"]).get("content", "") or "secret password" in r.get("content", "") for r in results)
